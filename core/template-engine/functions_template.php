...
/**
*
* @package WeBid from phpBB3 Modified for use with Dream Forgery
...
class template_compile
{
		...
		for ($curr_tb = 0, $tb_size = sizeof($blocks); $curr_tb < $tb_size; $curr_tb++)
		{
			...
			switch ($block_val[1])
			{
				...
				case 'INCLUDE':
					...
					if (!preg_match('#\{([a-zA-Z0-9_-]+)\}#is', $temp))
					{
	...
	*/
	function compile_var_tags(&$text_blocks)
	{
		...
		preg_match_all('#\{((?:[a-zA-Z0-9\-_]+\.)+)(\$)?([a-zA-Z0-9\-_]+)\}#', $text_blocks, $varrefs, PREG_SET_ORDER);
		...
		{
			$text_blocks = preg_replace('#\{L_([a-zA-Z0-9\-_]*)\}#is', "<?php echo ((isset(\$this->_rootref['L_\\1'])) ? \$this->_rootref['L_\\1'] : ((isset(\$MSG['\\1'])) ? \$MSG['\\1'] : '{ L_\\1 }')); ?>", $text_blocks);
		}

		// Handle remaining varrefs
		$text_blocks = preg_replace('#\{([a-zA-Z0-9_-]+)\}#is', "<?php echo (isset(\$this->_rootref['\\1'])) ? \$this->_rootref['\\1'] : ''; ?>", $text_blocks);
		$text_blocks = preg_replace('#\{([a-zA-Z0-9_-]+)\(([a-zA-Z0-9]+)\)\}#is', "<?php echo (isset(\$this->_rootref['\\1'][\\2])) ? \$this->_rootref['\\1'][\\2] : ''; ?>", $text_blocks);
		$text_blocks = preg_replace('#\{\$([a-zA-Z0-9_-]+)\}#is', "<?php echo (isset(\$this->_tpldata['DEFINE']['.']['\\1'])) ? \$this->_tpldata['DEFINE']['.']['\\1'] : ''; ?>", $text_blocks);
	...
	function compile_tag_if ($tag_args, $elseif)
	{
		...
		for ($i = 0, $size = sizeof($tokens); $i < $size; $i++)
		{
			...
			switch ($token)
			{
				...
				default:
					if (preg_match('#^((?:[a-zA-Z0-9\-_]+\.)+)?(\$)?(?=[a-zA-Z])([a-zA-Z0-9\-_]+)#s', $token, $varrefs))
					{
						$token = (!empty($varrefs[1])) ? $this->generate_block_data_ref(substr($varrefs[1], 0, -1), true, $varrefs[2]) . '[\'' . $varrefs[3] . '\']' : (($varrefs[2]) ? '$this->_tpldata[\'DEFINE\'][\'.\'][\'' . $varrefs[3] . '\']' : '$this->_rootref[\'' . $varrefs[3] . '\']');
					}
					else if (preg_match('#^\.((?:[a-zA-Z0-9\-_]+\.?)+)$#s', $token, $varrefs))
					{
	...
	*/
	...
	{
		preg_match('#^((?:[a-zA-Z0-9\-_]+\.)+)?\$(?=[a-zA-Z])([a-zA-Z0-9_\-]*)(?: = (\'?)([^\']*)(\'?))?$#', $tag_args, $match);
	...
	*/
	...
	{
		// add variables
		preg_match_all('#\{([a-zA-Z0-9_-]+)\}#is', $tag_args, $matches);
